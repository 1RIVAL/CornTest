...
class HolisticDetector():
    ...
    def findHolistic(self, img, draw=True):
        ...
        if self.results.pose_landmarks: # 생략 가능?!
    ...
    def findPoseLandmark(self, img, draw=True):
        ...
        if self.results.pose_landmarks:
            ...
            for id, lm in enumerate(myHolistic.landmark):
                # print(id,lm)
                h, w, c = img.shape
                cx, cy, cz = int(lm.x * w), int(lm.y * h), int(lm.z * (w + h) / 2) # z 계산 알고리즘? -  z 좌표를 화면의 크기에 상대적으로 나타내어 객체의 깊이를 추정하고 표시
                # print(id, cx, cy)
                # print(cz)
                xList.append(cx)
                yList.append(cy)
                self.pose_lmList.append([id, cx, cy, cz])
                #self.pose_lmList.append([cx, cy, cz])
    ...
    def findFaceLandmark(self, img, draw=True):
        ...
        if self.results.face_landmarks:
            ...
            for id, lm in enumerate(myHolistic.landmark):
                # print(id,lm)
                h, w, c = img.shape
                cx, cy, cz = int(lm.x * w), int(lm.y * h), int(lm.z * (w + h) / 2) # z 계산 알고리즘?
                # print(id, cx, cy)
                xList.append(cx)
                yList.append(cy)
                self.face_lmList.append([id, cx, cy, cz])
                #self.face_lmList.append([cx, cy, cz])
